...
def douban_job(douban_client: DoubanMoviesClient, counts: int, report_generator, notifier):
    LOG.info("[开始执行定时任务]豆瓣最新热门电影")
    ...
    # 获取当前日期，并格式化为 'YYYY-MM-DD' 格式
    date = datetime.now().strftime('%Y-%m-%d')
    notifier.notify_douban_report(date, report)
    LOG.info(f"[定时任务执行完毕]")
...
def main():
    ...
    douban_job(douban_client, 20, report_generator, notifier)
    ...
    # 安排 hn_daily_job 每天早上10点执行一次
    schedule.every().day.at("09:00").do(douban_job, douban_client, 20, report_generator, notifier)
