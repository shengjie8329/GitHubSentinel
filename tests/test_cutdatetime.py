...
# -*- coding: utf-8 -*-
"""
@Time   :2024/8/21 15:21
@Author :lancelot.sheng
@File   :test_cutdatetime.py
"""
import json
...
def filter_json_by_date_start(json_str, cutoff_date_str):
    # 解析输入的JSON字符串
    data = json.loads(json_str)
...
print(str(convert_to_date(cur)))


today = datetime.now().date()
print(str(convert_to_date(today.isoformat())))

print(convert_to_date(cur).strftime("%Y-%m-%dT%H:%M:%SZ"))
cutoff_date_str = convert_to_date(cur).strftime("%Y-%m-%dT%H:%M:%SZ")
filtered_json_str = filter_json_by_date(json_str, cutoff_date_str)
